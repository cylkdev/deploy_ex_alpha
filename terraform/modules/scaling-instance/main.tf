...
resource "aws_lb_target_group" "ec2_lb_instance_target_group" {
...
resource "aws_lb_target_group_attachment" "ec2_lb_instance_target_group_attachment" {
  count = var.enable_elb && var.attach_target_group ? 1 : 0

  target_group_arn = aws_lb_target_group.ec2_lb_instance_target_group[0].arn
...
resource "aws_lb" "ec2_lb" {
  ...
  name               = format("%s-%s-%s", local.inventory_group_kebab_case, var.environment, "lb")
...
resource "aws_lb_listener" "ec2_lb_listener" {
  ...
  protocol = aws_lb_target_group.ec2_lb_instance_target_group[0].protocol
